 ...
 */
public class attendance_summary extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Attendance Summary</u></html>");
    ...
    public final void load_list_table() {
        ...
        s2.addColumn("Name");
        s2.addColumn("Total Days");
        s2.addColumn("Present");
        s2.addColumn("Absent");
        s2.addColumn("%");
        jTable1.setModel(s2);
        jTable1.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        jTable1.getColumnModel().getColumn(0).setPreferredWidth(110);
        jTable1.getColumnModel().getColumn(1).setPreferredWidth(324);
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            boolean selva = false;
            String query;
            query = "select sid,sname,count(sid),sum(value) from atten_entry where dat between '" + lk + "' and '" + lk1 + "' group by sid order by sname";
            r = util.doQuery(query);
            while (r.next()) {
                int total = r.getInt(3);
                int present = r.getInt(4);
                int absent = total - present;
                double per = (present * 100) / total;
                s2.addRow(new Object[]{r.getString(1), r.getString(2), total, present, absent, (int) per + "%"});
                selva = true;
            }
            if (selva == true) {
                h1.setEnabled(false);
    ...
    public attendance_summary(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Attendance Summary");
