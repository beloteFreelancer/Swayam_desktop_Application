 ...
 */
public class estimate_summary_daywise extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Day Wise Estimate Summary</u></html>");
        setTitle("Day Wise Estimate Summary");
    ...
    public final void load_list_table() {
        ...
        jTable1.getColumnModel().getColumn(1).setPreferredWidth(174);
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            String query;
            query = "select date_format(dat,'%d/%m/%Y'),min(billno),max(billno),count(billno),sum(cash),sum(card),sum(credit),sum(upi),sum(others),sum(net) from estimate where dat between '" + lk + "' and '" + lk1 + "' group by dat order by dat";
            r = util.doQuery(query);
            while (r.next()) {
                String cash = String.format("%." + hmany + "f", r.getDouble(5));
                String card = String.format("%." + hmany + "f", r.getDouble(6));
                String credit = String.format("%." + hmany + "f", r.getDouble(7));
                String upi = String.format("%." + hmany + "f", r.getDouble(8));
            ...
            double cash = 0, card = 0, credit = 0, upi = 0, others = 0, net = 0;
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                cash = cash + Double.parseDouble(jTable1.getValueAt(i, 3).toString());
                card = card + Double.parseDouble(jTable1.getValueAt(i, 4).toString());
                credit = credit + Double.parseDouble(jTable1.getValueAt(i, 5).toString());
                upi = upi + Double.parseDouble(jTable1.getValueAt(i, 6).toString());
                others = others + Double.parseDouble(jTable1.getValueAt(i, 7).toString());
    ...
    public estimate_summary_daywise(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Day Wise Estimate Summary");
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            try (FileWriter f = new FileWriter(new File(folder + "/Drafts/" + file_name + ".xls"))) {
                f.write("Day Wise Estimate Summary:\n");
