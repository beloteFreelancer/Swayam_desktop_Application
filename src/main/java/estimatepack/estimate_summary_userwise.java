 ...
 */
public class estimate_summary_userwise extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            String query;
            query = "select cashier,count(billno),sum(cash),sum(card),sum(credit),sum(upi),sum(others),sum(net) from estimate where dat between '" + lk + "' and '" + lk1 + "' group by cashier order by cashier";
            r = util.doQuery(query);
            while (r.next()) {
                String cash = String.format("%." + hmany + "f", r.getDouble(3));
                String card = String.format("%." + hmany + "f", r.getDouble(4));
                String credit = String.format("%." + hmany + "f", r.getDouble(5));
                String upi = String.format("%." + hmany + "f", r.getDouble(6));
                String others = String.format("%." + hmany + "f", r.getDouble(7));
                String net = String.format("%." + hmany + "f", r.getDouble(8));

                s2.addRow(new Object[]{r.getString(1), r.getString(2), cash, card, credit, upi, others, net});
                selva = true;
            }

            double cash = 0, card = 0, credit = 0, upi = 0, others = 0, net = 0;
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                cash = cash + Double.parseDouble(jTable1.getValueAt(i, 2).toString());
                card = card + Double.parseDouble(jTable1.getValueAt(i, 3).toString());
                credit = credit + Double.parseDouble(jTable1.getValueAt(i, 4).toString());
                upi = upi + Double.parseDouble(jTable1.getValueAt(i, 5).toString());
                others = others + Double.parseDouble(jTable1.getValueAt(i, 6).toString());
    ...
    public estimate_summary_userwise(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Cashier Wise Estimate Summary");
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            try (FileWriter f = new FileWriter(new File(folder + "/Drafts/" + file_name + ".xls"))) {
                f.write("Cashier Wise Estimate Summary:\n");
