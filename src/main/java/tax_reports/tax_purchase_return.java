...
public class tax_purchase_return extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            query = "select distinct cname,billno,date_format(bdate,'%d/%m/%Y'),grn,ttype,bdate from preturn_items where bdate between '"
            ...
            for (int i = 0; i < billno.size(); i++) {
                ...
                query = "select sum(sub) from preturn_items where bdate between '" + lk + "' and '" + lk1
                ...
                taxamt = 0;
                total = 0;
                query = "select sum(sub),sum(taxamt),sum(total) from preturn_items where bdate between '" + lk
