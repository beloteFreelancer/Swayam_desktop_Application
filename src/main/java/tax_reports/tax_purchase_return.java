 ...
 */
public class tax_purchase_return extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>GSTR -Purchase Return</u></html>");
    }
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            query = "select distinct cname,billno,date_format(bdate,'%d/%m/%Y'),grn,ttype,bdate from preturn_items where bdate between '" + lk + "' and '" + lk1 + "' order by bdate,grn";
            ...
            for (int i = 0; i < billno.size(); i++) {
                ...
                query = "select sum(sub) from preturn_items where bdate between '" + lk + "' and '" + lk1 + "' and taxp='" + taxp + "' and billno='" + billno.get(i).toString() + "'";
                r = util.doQuery(query);
                while (r.next()) {
                    value = r.getDouble(1);
                }
...
//5 Percentage Tax
                ...
                taxamt = 0;
                total = 0;
                query = "select sum(sub),sum(taxamt),sum(total) from preturn_items where bdate between '" + lk + "' and '" + lk1 + "' and taxp='" + taxp + "' and billno='" + billno.get(i).toString() + "'";
                r = util.doQuery(query);
                while (r.next()) {
                    value = r.getDouble(1);
...
//28 Percentage Ends
    ...
    public tax_purchase_return(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("GSTR -Purchase Return");
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        if (s2.getRowCount() <= 0) {
            ...
            new ExcelUtilGST(h1.getText(), h2.getText()).GenerateGstrPurchaseReturnWorkbook();
        }
