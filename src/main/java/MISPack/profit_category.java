 ...
 */
public class profit_category extends javax.swing.JInternalFrame {
    ...
    public final void load_list_table() {
        ...
        s2.addColumn("Qty");
        s2.addColumn("Sales Value");
        s2.addColumn("Purchase Value");
        s2.addColumn("Profit Amount");
        jTable1.setModel(s2);
        ...
        jTable1.getColumnModel().getColumn(0).setPreferredWidth(304);
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            boolean selva = false;
            String query;
            query = "select cat,sum(quan),sum(sub),sum(cost_rate),sum(profit) from sales_items a,item b where dat between '" + lk + "' and '" + lk1 + "' and a.ino=b.ino group by cat order by sum(profit) desc";
            r = util.doQuery(query);
            while (r.next()) {
                String sub = String.format("%." + hmany + "f", r.getDouble(3));
                String cost_rate = String.format("%." + hmany + "f", r.getDouble(4));
                String profit = String.format("%." + hmany + "f", r.getDouble(5));

                s2.addRow(new Object[]{r.getString(1), r.getString(2), sub, cost_rate, profit});
                selva = true;
            }
            double sub = 0, cost_rate = 0, profit = 0;
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                sub = sub + Double.parseDouble(jTable1.getValueAt(i, 2).toString());
                cost_rate = cost_rate + Double.parseDouble(jTable1.getValueAt(i, 3).toString());
                profit = profit + Double.parseDouble(jTable1.getValueAt(i, 4).toString());
            }
    ...
    public profit_category(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Category Wise Profit");
        ...
        jLabel12.setText("Info: Profit Calculations not includes Tax Amount, Transport Amount , Other Charges");
        getContentPane().add(jLabel12);
        jLabel12.setBounds(10, 560, 600, 30);
