...
public final class profit_itemwise_barcode extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            query = "select a.ino,a.iname,sum(quan),sum(sub),sum(cost_rate),sum(profit) from sales_items a,item b where dat between ? and ? and a.barcode=? and a.ino=b.ino group by a.ino, a.iname order by sum(profit) desc";
            ps = conn.prepareStatement(query);
            ps.setString(1, lk);
            ps.setString(2, lk1);
            ps.setString(3, h4.getText());
    ...
    final void get_sug1() {
        try {
            int count = 0;
            String query = "select distinct iname from sales_items";
            ResultSet set = util.doQuery(query);
            while (set.next()) {
                count = count + 1;
            }
            query = "select distinct iname from sales_items";
    ...
    public profit_itemwise_barcode(DataUtil util) {
        initComponents();
        this.util = util;
        load_list_table();
        button_short();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Barcode Wise Profit");
        ...
        generatebutton.setBounds(810, 70, 190, 30);
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            String file_name = "ProfitByBarcode.xlsx";
