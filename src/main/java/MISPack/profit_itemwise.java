...
public final class profit_itemwise extends javax.swing.JInternalFrame {
    ...
    public final void load_list_table() {
        ...
        jTable1.getColumnModel().getColumn(5).setPreferredWidth(154);
        String Ta = "Arial";
        int Bold = 0, size = 14;
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select a.ino,a.iname,sum(quan),sum(sub),sum(cost_rate),sum(profit) from sales_items a,item b where dat between ? and ? and a.ino=b.ino group by a.ino, a.iname order by sum(profit) desc";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select a.ino,a.iname,sum(quan),sum(sub),sum(cost_rate),sum(profit) from sales_items a,item b where dat between ? and ? and cat=? and a.ino=b.ino group by a.ino, a.iname order by sum(profit) desc";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            ...
            r = ps.executeQuery();
            while (r.next()) {
                String sub = String.format("%." + hmany + "f", r.getDouble(4));
            ...
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                ...
                cost_rate = cost_rate + Double.parseDouble(jTable1.getValueAt(i, 4).toString());
                profit = profit + Double.parseDouble(jTable1.getValueAt(i, 5).toString());
            }
    ...
    public profit_itemwise(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Item Wise Profit");
        ...
        jLabel14.setBounds(350, 40, 70, 30);

        jLabel15.setFont(new java.awt.Font("Arial", 0, 14)); // NOI18N
        jLabel15.setText("Date from");
        getContentPane().add(jLabel15);
        jLabel15.setBounds(10, 40, 70, 30);
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            String file_name = "ItemWiseProfit.xlsx";
