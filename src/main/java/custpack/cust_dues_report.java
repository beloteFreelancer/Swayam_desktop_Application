 ...
 */
public final class cust_dues_report extends javax.swing.JInternalFrame {
    ...
    void button_short() {
        ...
        titlelablel.setText("<html><u>Customer Dues Report</u></html>");

        setTitle("Customer Dues Report");
    ...
    void load_report() {
        try {
            ...
            String query;
            if (all.isSelected()) {
                query = "select billno,date_format(dat,'%d/%m/%Y'),date_format(ddate,'%d/%m/%Y'),cid,cname,tot,paid from cust_bal where tot-paid>0 order by dat,billno";
            } else {
                query = "select billno,date_format(dat,'%d/%m/%Y'),date_format(ddate,'%d/%m/%Y'),cid,cname,tot,paid from cust_bal where cid='" + h4.getText() + "' and tot-paid>0 order by dat,billno";
            }
    ...
    void get_cid() {
        try {
            int count = 0;
            String query = "select distinct cname from cust_bal where tot-paid>0";
            r = util.doQuery(query);
            while (r.next()) {
                count = count + 1;
            }
            query = "select distinct cname from cust_bal where tot-paid>0";
            r = util.doQuery(query);
    ...
    void get_cname() {
        try {
            String query = "select cid from cust where cname='" + h3.getSelectedItem() + "' ";
            r = util.doQuery(query);
            while (r.next()) {
    ...
    public cust_dues_report(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Customer Dues Report");
        ...
        generatebutton.setBounds(680, 50, 180, 30);
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            String file_name = "CustomerDuesReport.xlsx";
