...
public class pay_bill_report extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            String query;
            query = "select sno,date_format(dat,'%d/%m/%Y'),date_format(dfrom,'%d/%m/%Y'),date_format(dto,'%d/%m/%Y'),count(sno),sum(gross),sum(lpay),sum(lopay),sum(net),last from pay_bill where dat between '"
                    + lk + "' and '" + lk1 + "' group by sno,dat,dfrom,dto,last order by sno";
            ...
            while (r.next()) {
                ...
                s2.addRow(new Object[] { r.getString(1), r.getString(2), r.getString(3) + " - " + r.getString(4),
                        r.getString(5), gross2, lpay2, lopay2, net2, r.getString(10) });
                selva = true;
            }
            gross = 0;
            lpay = 0;
            ...
            if (selva == true) {
                ...
                s2.addRow(new Object[] { "", "", "TOTAL:" + (jTable1.getRowCount() - 1), "", "" + gross2, lpay2, lopay2,
                        net2, "" });
