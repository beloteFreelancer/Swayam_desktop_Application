 ...
 */
public class pay_bill_report extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Salary Report</u></html>");
    ...
    public final void load_list_table() {
        ...
        s2.addColumn("Date");
        s2.addColumn("Salary Period");
        s2.addColumn("Total Staffs");
        s2.addColumn("Gross Pay");
        s2.addColumn("Advance");
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            boolean selva = false;
            double gross, lpay, lopay, net;

            String query;
            query = "select sno,date_format(dat,'%d/%m/%Y'),date_format(dfrom,'%d/%m/%Y'),date_format(dto,'%d/%m/%Y'),count(sno),sum(gross),sum(lpay),sum(lopay),sum(net),last from pay_bill where dat between '" + lk + "' and '" + lk1 + "' group by sno,dat,dfrom,dto,last order by sno";
            ...
            while (r.next()) {
                ...
                String lpay2 = String.format("%." + hmany + "f", lpay);
                String lopay2 = String.format("%." + hmany + "f", lopay);
                String net2 = String.format("%." + hmany + "f", net);
                s2.addRow(new Object[]{r.getString(1), r.getString(2), r.getString(3) + " - " + r.getString(4), r.getString(5), gross2, lpay2, lopay2, net2, r.getString(10)});
                selva = true;
            }
            gross = 0;
            lpay = 0;
            ...
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                gross = gross + Double.parseDouble(jTable1.getValueAt(i, 4).toString());
                lpay = lpay + Double.parseDouble(jTable1.getValueAt(i, 5).toString());
                lopay = lopay + Double.parseDouble(jTable1.getValueAt(i, 6).toString());
            ...
            if (selva == true) {
                ...
                s2.addRow(new Object[]{"", "", "TOTAL:" + (jTable1.getRowCount() - 1), "", "" + gross2, lpay2, lopay2, net2, ""});
    ...
    public pay_bill_report(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Salary Report");
    ...
    private void viewbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewbuttonActionPerformed
        pay_bill_generation oe = new pay_bill_generation(util);
