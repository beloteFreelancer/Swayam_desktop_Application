...
public class print_thermal_summary_return {
    ...
    public void Report(DataUtil util, String billno, String drive, String folder, String billformat) {
        try {
            ...
            query = "select sum(sub),sum(taxamt) from sreturn_items where billno='" + billno + "' and taxp='5' ";
            r = util.doQuery(query);
            while (r.next()) {
                value5 = r.getDouble(1);
                gst5 = r.getDouble(2);
            }
            query = "select sum(sub),sum(taxamt) from sreturn_items where billno='" + billno + "' and taxp='12' ";
            r = util.doQuery(query);
            while (r.next()) {
                value12 = r.getDouble(1);
                gst12 = r.getDouble(2);
            }
            query = "select sum(sub),sum(taxamt) from sreturn_items where billno='" + billno + "' and taxp='18' ";
            r = util.doQuery(query);
            while (r.next()) {
                value18 = r.getDouble(1);
                gst18 = r.getDouble(2);
            }
            query = "select sum(sub),sum(taxamt) from sreturn_items where billno='" + billno + "' and taxp='28' ";
            r = util.doQuery(query);
            while (r.next()) {
