...
public class print_a4_half {
    ...
    public void Report(DataUtil util, String billno, String drive, String folder, String billformat) {
        ...
        try {
            ...
            String date = "", cid = "", tax = "", salesman = "";
            double sub = 0, dis = 0, addamt = 0, net = 0, taxamt = 0, quans = 0;
            query = "select date_format(dat,'%d/%m/%Y'),quans,sub,disamt,addamt,net,cid,taxamt,tax from sales where billno='" + billno + "'";
            ...
            if (ad3.length() > 1) {
                parameters.put("parameter15", "" + ad3);
            }
            if (mobile.length() > 1) {
                parameters.put("parameter16", "Contact Nos: " + mobile);
            }
            if (ctin.length() > 1) {
            ...
            while (r.next()) {
                ...
                String free2 = String.format("%." + hmany1 + "f", free);
                String[] spli5 = free2.split("\\.");
                int wh5 = Integer.parseInt(spli5[1]);
                if (wh5 <= 0) {
                    free2 = spli5[0];
                }
                selRomJasper.setField6("" + free2);
                ...
                selRomJasper.setField9("" + sgst_per2);
            ...
            // Refactored to use standard Sales_A5.jrxml (formerly A4Half-II/III/IV)
            // Removed reference to missing A4Half-V.jrxml
            jasperReport = JasperReportCompiler.compileReport("/JasperFiles/A4_Half/Sales_A5.jrxml");
