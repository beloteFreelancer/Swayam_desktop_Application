 ...
 */
public final class sreturn extends javax.swing.JInternalFrame {
    ...
    String username = "", utype = "", drive = "", folder = "", version = "", retail_format = "", wholesale_format = "",
            round_option = "Yes";
    String scode = "", sname = "", ttype = "", stock_bill, less_prate = "", auto_sms = "", tax = "Local",
            entry_mode = "Auto";
    double max_rdis = 0, max_wdis = 0;
    int hmany = 2, hmany1 = 3;

    // public sreturn(DataUtil util) {
    // this.util = util;
    // initComponents();
    // get_defaults();
    // button_short();
    ...
    void display_terminal() {
        try {
            try (FileInputStream m = new FileInputStream(folder + "/Config_Files/terminal.properties")) {
                ...
                String term = p.getProperty("terminal");
                System.out.println("Loaded location: " + loc);
                if (loc == null || loc.trim().isEmpty()) {
                    loc = "DefaultLocation";
                    System.out.println(
                            "Warning: location property is missing or empty in terminal.properties. Using DefaultLocation.");
                }
                if (term == null || term.trim().isEmpty()) {
                    term = "DefaultTerminal";
                    System.out.println(
                            "Warning: terminal property is missing or empty in terminal.properties. Using DefaultTerminal.");
                }
                h4.setText(loc);
                h5.setText(term);
            }
        } catch (IOException e) {
            System.out.println("Error reading terminal.properties: " + e.getMessage());
        }
    }
    ...
    void get_billno() {
        try {
            int sno = 1;
            String query = "select max(billno) from sreturn";
            ResultSet r = util.doQuery(query);
            boolean selva = false;
            while (r.next()) {
                selva = true;
        ...
        try {
            ...
            if (selvakumar == false) {
                s2.addRow(new Object[] { jTable1.getRowCount() + 1, ino, iname, quan2, mrp2, price2, amount2, disp,
                        disamt, sub, taxp, taxamt, total, udes, barcode, hsn, tax, entry, prate, 0, iname1 });
                serial_num();
            } else {
    ...
    void save() {
        try {
            ...
            boolean selva = false;
            String query = "select billno from sreturn where billno='" + h1.getText() + "'";
            ResultSet r = util.doQuery(query);
            while (r.next()) {
                selva = true;
            }
            ...
            if (pricel.getText().equals("Wholesale")) {
                ...
                if (ctype.equals("Retail")) {
                    JOptionPane.showMessageDialog(this,
                            "<html>For Wholesale Bill First Create Customer Master<br>, Create as <b> 'Credit Customer'</b></html>",
                            "Invalid Wholesale Customer!", JOptionPane.ERROR_MESSAGE);
                    h24.requestFocus();
                    return;
                }
            ...
            try {
                ...
                psSreturn.setString(3, time);
                psSreturn.setString(4, location);
                psSreturn.setString(5, cashier);
                psSreturn.setString(6, terminal);
                psSreturn.setString(7, items);
                psSreturn.setString(8, quans);
                psSreturn.setString(9, sub);
                psSreturn.setString(10, disp);
                psSreturn.setString(11, disamt);
                psSreturn.setString(12, gross);
                psSreturn.setString(13, taxamt);
                psSreturn.setString(14, addamt);
                psSreturn.setString(15, round);
                psSreturn.setDouble(16, net);
                psSreturn.setString(17, pby);
                psSreturn.setString(18, paid);
                psSreturn.setString(19, bal);
                psSreturn.setString(20, cash);
                psSreturn.setString(21, card);
                psSreturn.setString(22, credit);
                psSreturn.setString(23, others);
                psSreturn.setString(24, upi);
                psSreturn.setString(25, price_type);
                psSreturn.setString(26, ttype);
                psSreturn.setString(27, tax);
                psSreturn.setString(28, cid);
                psSreturn.setString(29, cardno);
                psSreturn.setString(30, cname);
                ...
                if (bb == JOptionPane.YES_OPTION) {
                    ...
                    new print_class_return().get_print(util, billno, drive, folder, billformat);
                }
                form_clear();
    ...
    public void view(String billno) {
        try {
            String query = "select distinct billno from sreturn where billno='" + billno + "'";
            ...
            if (selva == false) {
                ...
                query = "select billno,date_format(dat,'%d/%m/%Y'),tim,location,terminal,cashier,items,quans,sub,disp,disamt,gross,taxamt,addamt,round,pby,net,paid,bal,cash,card,others,upi,price_type,tax_type,tax,cid,cardno,mobile,cname from sreturn where billno='"
                        + billno + "'";
                ...
                while (set1.next()) {
                    ...
                    pricel.setText(set1.getString(24));
                    ttype = set1.getString(25);
                    tax = set1.getString(26);

                    h21.setText(set1.getString(27));
                    h22.setText(set1.getString(28));
                    h23.setText(set1.getString(29));
                    h24.setText(set1.getString(30));

                    String net2 = String.format("%." + hmany + "f", net);
                    netl.setText(net2);
                }

                query = "select serial,ino,iname,quan,mrp,price,amount,disp,disamt,sub,taxp,taxamt,total,udes,barcode,hsn,tax,entry,cost_rate,profit,iname1 from sreturn_items where billno='"
                        + billno + "'";
                set1 = util.doQuery(query);
                while (set1.next()) {
                    ...
                    s2.addRow(new Object[] { set1.getString(1), set1.getString(2), set1.getString(3), quan2, mrp, price,
                            ...
                            set1.getString(20), set1.getString(21) });
                }
    ...
    void get_price_change() {
        if (s2.getRowCount() > 0) {
            JOptionPane.showMessageDialog(this, "<html><h1>Items Already Entered!</h1></html>", "Invalid",
                    JOptionPane.ERROR_MESSAGE);
            h17.requestFocus();
            return;
        }
    ...
    void alter_sreturn() {
        try {
            ...
            String query = "select barcode,ino,iname,quan,entry from sreturn_items where billno='" + billno + "'";
            ResultSet set1 = util.doQuery(query);
            boolean selva = false;
            while (set1.next()) {
                selva = true;
            ...
            if (selva == true) {
                for (int i = 0; i < barcode.size(); i++) {
                    query_batch.add("update stock set quan=quan-" + quan.get(i) + " where barcode='" + barcode.get(i)
                            + "' and ino='" + ino.get(i) + "' and iname='" + iname.get(i) + "' and entry='"
                            + entry.get(i) + "'");
                }
            } // selva true ends
            query_batch.add("delete from sreturn where billno='" + billno + "'");
            query_batch.add("delete from sreturn_items where billno='" + billno + "'");
            if (h16.getSelectedItem().equals("Credit")) {
    ...
    private void initComponents() {
        ...
        jLabel18.setBounds(0, 450, 110, 30);
    ...
    private void nextbuttonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_nextbuttonActionPerformed
        try {
            ...
            String query = "select billno from sreturn where billno > '" + grn + "' order by billno limit 1";
            ResultSet set1 = util.doQuery(query);
            boolean selva = false;
            String search_grn = "";
            while (set1.next()) {
    ...
    private void prebuttonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_prebuttonActionPerformed
        try {
            ...
            String query;
            if (grn.equalsIgnoreCase("--")) {
                query = "select max(billno) from sreturn";
            } else {
                query = "select billno from sreturn where billno < '" + grn + "' order by billno desc limit 1";
            }
    ...
    private void printbuttonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_printbuttonActionPerformed
        ...
        new print_class_return().get_print(util, h1.getText(), drive, folder, billformat);
    ...
    private void jTable3MouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_jTable3MouseClicked
        if (jTable3.getRowCount() > 0) {
            ...
            String rprice = jTable3.getValueAt(jTable3.getSelectedRow(), 5).toString();
            String wprice = jTable3.getValueAt(jTable3.getSelectedRow(), 6).toString();
            if (pricel.getText().equals("Retail")) {
                h19.setText(rprice);
            } else {
                h19.setText(wprice);
