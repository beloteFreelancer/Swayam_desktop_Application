 ...
 */
public class purchase_summary_manu extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Manufacturer Wise Purchase Summary</u></html>");
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select manu,sum(quan),sum(amount) from purchase_items where bdate between ? and ? group by manu order by manu";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select manu,sum(quan),sum(amount) from purchase_items where bdate between ? and ? and manu=? group by manu order by manu";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
    ...
    final void get_cid() {
        try {
            int count = 0;
            Connection conn = util.getConnection();
            PreparedStatement ps = conn.prepareStatement("select count(distinct manu) from purchase_items");
            r = ps.executeQuery();
            while (r.next()) {
                count = r.getInt(1);
            }
            ps = conn.prepareStatement("select distinct manu from purchase_items");
            r = ps.executeQuery();
    ...
    public purchase_summary_manu(DataUtil util) {
        initComponents();
        this.util = util;
        button_short();
        load_list_table();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Manufacturer Wise Purchase Summary");
        ...
        jLabel5.setBounds(370, 50, 90, 30);
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            String file_name = "Manufacturer_Wise_Purchase_Summary.xlsx";
