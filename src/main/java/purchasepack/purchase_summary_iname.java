 ...
 */
public class purchase_summary_iname extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Item Wise Purchase Summary</u></html>");
    ...
    final void load_list_table() {
        ...
        jTable1.getColumnModel().getColumn(2).setPreferredWidth(244);
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select ino,iname,sum(quan),sum(amount) from purchase_items where bdate between ? and ? group by ino order by iname";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select ino,iname,sum(quan),sum(amount) from purchase_items where bdate between ? and ? and iname=? group by ino order by iname";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
                ps.setString(3, h3.getSelectedItem().toString());
            }
            r = ps.executeQuery();
            while (r.next()) {
                amount = r.getDouble(4);
                String amount1 = String.format("%." + hmany + "f", amount);
                s2.addRow(new Object[]{r.getString(1), r.getString(2), r.getString(3), amount1});
                selva = true;
            }
            amount = 0;
            double bills = 0;
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                bills = bills + Double.parseDouble(jTable1.getValueAt(i, 2).toString());
                amount = amount + Double.parseDouble(jTable1.getValueAt(i, 3).toString());
            }
            String amount1 = String.format("%." + hmany + "f", amount);
            if (selva == true) {
    ...
    final void get_cid() {
        try {
            int count = 0;
            Connection conn = util.getConnection();
            PreparedStatement ps = conn.prepareStatement("select count(distinct iname) from purchase_items");
            r = ps.executeQuery();
            while (r.next()) {
                count = r.getInt(1);
            }
            ps = conn.prepareStatement("select distinct iname from purchase_items");
            r = ps.executeQuery();
    ...
    void get_cname() {
        try {
            cnamel.setText("");
            Connection conn = util.getConnection();
            PreparedStatement ps = conn.prepareStatement("select distinct ino from purchase_items where iname=?");
            ps.setString(1, h3.getSelectedItem().toString());
            r = ps.executeQuery();
            while (r.next()) {
    ...
    public purchase_summary_iname(DataUtil util) {
        initComponents();
        this.util = util;
        button_short();
        load_list_table();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Item Wise Purchase Summary");
        ...
        jScrollPane1.setBounds(0, 112, 1000, 450);
        ...
        generatebutton.setBounds(810, 80, 190, 30);
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            String file_name = "Item_Wise_Purchase_Summary.xlsx";
