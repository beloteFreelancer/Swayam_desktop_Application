...
public class Cashbook_Summary {
    ...
    public void Report(DataUtil util, String dfrom, String dto) {
        ...
        try {
            ...
            parameters.put("parameter2", "Cash Book Summary");
            parameters.put("parameter3", "(Date from " + dfrom + " to " + dto + ")");

            double sales = 0;
            query = "select sum(cash) from sales where dat< '" + dfrom1 + "'";
            r = util.doQuery(query);
            while (r.next()) {
                sales = r.getDouble(1);
            }
            double preturn = 0;
            query = "select sum(net) from preturn where dat< '" + dfrom1 + "' and pby='Cash' ";
            r = util.doQuery(query);
            while (r.next()) {
                preturn = r.getDouble(1);
            }
            double cust_pay = 0;
            query = "select sum(amount) from cust_pay where dat< '" + dfrom1 + "' and pby='Cash' ";
            r = util.doQuery(query);
            while (r.next()) {
            ...
            double purchase = 0;
            query = "select sum(net) from purchase where dat< '" + dfrom1 + "' and pby='Cash' ";
            r = util.doQuery(query);
            while (r.next()) {
                purchase = r.getDouble(1);
            }
            double sreturn = 0;
            query = "select sum(cash) from sreturn where dat< '" + dfrom1 + "'";
            r = util.doQuery(query);
            while (r.next()) {
                sreturn = r.getDouble(1);
            }
            double ven_pay = 0;
            query = "select sum(amount) from ven_pay where dat< '" + dfrom1 + "' and pby='Cash' ";
            r = util.doQuery(query);
            while (r.next()) {
            ...
            query = "select sum(cash) from sales where dat between '" + dfrom1 + "' and '" + dto1 + "' ";
            r = util.doQuery(query);
            while (r.next()) {
            ...
            query = "select sum(cash) from sreturn where dat between '" + dfrom1 + "' and '" + dto1 + "' ";
            r = util.doQuery(query);
            while (r.next()) {
                double amt = r.getDouble(1);
                if (amt > 0) {
            ...
            while (r.next()) {
                double amt = r.getDouble(1);
                if (amt > 0) {
                    iname.add("Contra:  from Bank A/c");
                    debit.add(0);
            ...
            jasperViewer.setTitle("Cash Book Summary");
