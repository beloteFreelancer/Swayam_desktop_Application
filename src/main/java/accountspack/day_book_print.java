...
public class day_book_print {
    ...
    public void Report(DataUtil util, String dfrom, String dto) throws ParseException {
        try {
            ...
            String voucherDebitOpeningQuery = "SELECT COALESCE(SUM(amount), 0) FROM account_voucher WHERE dat < '"
                    + dfrom1 + "' AND entry = 'Debit'";
            ...
            String voucherCreditOpeningQuery = "SELECT COALESCE(SUM(amount), 0) FROM account_voucher WHERE dat < '"
                    + dfrom1 + "' AND entry = 'Credit'";
            ...
            try {
                ...
                r = util.doQuery(query);
                while (r.next()) {
                    transactions.add(new Transaction(r.getDate("dat"), "SALE-" + r.getString("billno"),
                            r.getString("cname"), 0, r.getDouble("net"), "Sales"));
                }
                ...
                r = util.doQuery(query);
                while (r.next()) {
                    transactions.add(new Transaction(r.getDate("dat"), "PUR-" + r.getString("grn"),
                            r.getString("cname"), r.getDouble("net"), 0, "Purchase"));
                }
                ...
                r = util.doQuery(query);
                while (r.next()) {
                    transactions.add(new Transaction(r.getDate("dat"), "SRET-" + r.getString("billno"),
                            r.getString("cname"), r.getDouble("net"), 0, "Sales Return"));
                }
                ...
                r = util.doQuery(query);
                while (r.next()) {
                    transactions.add(new Transaction(r.getDate("dat"), "PRET-" + r.getString("grn"),
                            r.getString("cname"), 0, r.getDouble("net"), "Purchase Return"));
                }

                // Fetch Account Vouchers
                query = "SELECT dat, billno, whom, what, amount, entry FROM account_voucher WHERE dat BETWEEN '"
                        + dfrom1 + "' AND '" + dto1 + "'";
                r = util.doQuery(query);
                while (r.next()) {
                    ...
                    if ("Debit".equalsIgnoreCase(entry)) {
                        transactions.add(new Transaction(r.getDate("dat"), "VCH-" + r.getString("billno"), particulars,
                                amount, 0, "Voucher Debit"));
                    } else {
                        transactions.add(new Transaction(r.getDate("dat"), "VCH-" + r.getString("billno"), particulars,
                                0, amount, "Voucher Credit"));
                    }
                }

                // Fetch Cust Pay
                query = "SELECT dat, billno, cname, amount, remarks FROM cust_pay WHERE dat BETWEEN '" + dfrom1
                        + "' AND '" + dto1 + "'";
                r = util.doQuery(query);
                while (r.next()) {
                    ...
                    transactions.add(new Transaction(r.getDate("dat"), "CP-" + r.getString("billno"), particulars, 0,
                            r.getDouble("amount"), "Cust Receipt"));
                }

                // Fetch Ven Pay
                query = "SELECT dat, billno, cname, amount, remarks FROM ven_pay WHERE dat BETWEEN '" + dfrom1
                        + "' AND '" + dto1 + "'";
                r = util.doQuery(query);
                while (r.next()) {
                    ...
                    transactions.add(new Transaction(r.getDate("dat"), "VP-" + r.getString("billno"), particulars,
                            r.getDouble("amount"), 0, "Ven Payment"));
                }
                ...
                r = util.doQuery(query);
                while (r.next()) {
                    transactions.add(new Transaction(r.getDate("dat"), "PAY-" + r.getString("sno"),
                            "Salary - " + r.getString("cname"), r.getDouble("net"), 0, "Salary"));
                }
            ...
            java.awt.Image iconImage = icon != null ? icon.getImage() : null;
            if (iconImage != null) {
                jasperViewer.setIconImage(iconImage);
            }
