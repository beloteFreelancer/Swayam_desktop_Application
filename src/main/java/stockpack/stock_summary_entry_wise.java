 ...
 */
public class stock_summary_entry_wise extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Entry Wise Stock Summary</u></html>");
        setTitle("Entry Wise Stock Summary");
    ...
    public final void load_list_table() {
        ...
        jTable1.getColumnModel().getColumn(0).setPreferredWidth(294);
    ...
    void load_report() {
        try {
            ...
            if (h3.getSelectedItem().equals("Purchase")) {
                query = "select cat,sum(quan),sum(quan*prate),sum(quan*mrp),sum(quan*rprice),sum(quan*wprice) from stock where quan>0 and entry='purchase' group by cat order by cat";
            } else {
                query = "select cat,sum(quan),sum(quan*prate),sum(quan*mrp),sum(quan*rprice),sum(quan*wprice) from stock where quan>0 and entry='stock' group by cat order by cat";
            }
            ...
            while (r.next()) {
                ...
                s2.addRow(new Object[]{r.getString(1), r.getString(2), prate1, mrp1, srate1, wrate1});
                selva = true;
                prate = 0;
                mrp = 0;
                srate = 0;
            ...
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                quan = quan + Double.parseDouble(jTable1.getValueAt(i, 1).toString());
                prate = prate + Double.parseDouble(jTable1.getValueAt(i, 2).toString());
                mrp = mrp + Double.parseDouble(jTable1.getValueAt(i, 3).toString());
                srate = srate + Double.parseDouble(jTable1.getValueAt(i, 4).toString());
                wrate = wrate + Double.parseDouble(jTable1.getValueAt(i, 5).toString());
            }
            String prate1 = String.format("%." + hmany + "f", prate);
            String mrp1 = String.format("%." + hmany + "f", mrp);
            String srate1 = String.format("%." + hmany + "f", srate);
            ...
            if (selva == true) {
                ...
                s2.addRow(new Object[]{"TOTAL:" + (jTable1.getRowCount() - 1), quan, prate1, mrp1, srate1, wrate1});
    ...
    public stock_summary_entry_wise(DataUtil util) {
        initComponents();
    ...
    private void initComponents() {
        ...
        titlelablel.setText("Entry Wise Stock Summary");
    ...
    private void excelbuttonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_excelbuttonActionPerformed
        ...
        try {
            ...
            try (FileWriter f = new FileWriter(new File(folder + "/Drafts/" + file_name + ".xls"))) {
                f.write("Entry Wise Stock Summary:\n");
