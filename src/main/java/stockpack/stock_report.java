...
public class stock_report extends javax.swing.JInternalFrame {
    ...
    final void button_short() {
        ...
        titlelablel.setText("<html><u>Stock Report</u></html>");
    ...
    void load_report() {
        try {
            ...
            if (all.isSelected()) {
                query = "select ino,iname,sum(quan),sum(quan*prate),sum(quan*mrp),sum(quan*rprice),sum(quan*wprice) from stock where quan>0 group by ino order by ino";
            } else {
                query = "select ino,iname,sum(quan),sum(quan*prate),sum(quan*mrp),sum(quan*rprice),sum(quan*wprice) from stock where cat='"
                        + h3.getSelectedItem() + "' and quan>0 group by ino order by ino";
            }
            ...
            while (r.next()) {
                ...
                s2.addRow(
                        new Object[] { r.getString(1), r.getString(2), r.getString(3), prate1, mrp1, srate1, wrate1 });
                selva = true;
                prate = 0;
                mrp = 0;
                srate = 0;
    ...
    public stock_report(DataUtil util) {
        initComponents();
        this.util = util;
        button_short();
        load_list_table();
